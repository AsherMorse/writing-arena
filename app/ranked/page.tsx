...
import { WritingEditor } from '@/app/_components/WritingEditor';
import { PromptCard } from '@/app/_components/PromptCard';
import { 
  FeedbackProvider,
  WritingCard,
  ExpandableScoreBreakdown,
} from '@/app/_components/FeedbackDisplay';
import { FeedbackSidebar } from '@/app/_components/FeedbackSidebar';
import { LoadingOverlay } from '@/app/_components/LoadingOverlay';
import { Leaderboard } from '@/app/_components/Leaderboard';
import { WinningSubmissions } from '@/app/_components/WinningSubmissions';
import { RecommendedLessons } from '@/app/_components/RecommendedLessons';
import { ScrollShadow } from '@/app/_components/ScrollShadow';
import { ParchmentCard } from '@/app/_components/ParchmentCard';
import { ParchmentButton } from '@/app/_components/ParchmentButton';
import { ParchmentAccordion } from '@/app/_components/ParchmentAccordion';
import { getParchmentTextStyle } from '@/app/_components/parchment-styles';
import { RankedResultsModal } from '@/app/_components/RankedResultsModal';
import { Top3CelebrationModal } from '@/app/_components/Top3CelebrationModal';
...
export default function RankedPage() {
  ...
  const beginSelection = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/daily-prompt/options', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSelection = useCallback(async (selection: string) => {
    ...
    try {
      ...
      setIsLoadingInspiration(true);
      fetch('/api/generate-inspiration', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
          ...
          {phase === 'blocked' && blockStatus && (
            <div className="w-full max-w-2xl text-center space-y-8">
              ...
              <div className="flex justify-center gap-4">
                ...
                <ParchmentButton onClick={() => router.push('/study')} variant="golden">
                  Start Learning
                </ParchmentButton>
              </div>
            </div>
